...
import { DollarSign, Pen as Yen, Euro as EuroSign, PoundSterling } from 'lucide-react';
...
const Ticker: React.FC = () => {
  return (
    <div className="bg-accent-600 text-white py-1.5">
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex-1 overflow-hidden">
            <div className="animate-marquee whitespace-nowrap flex items-center space-x-8">
              {forexRates.map((rate) => (
                <div key={rate.code} className="inline-flex items-center space-x-2">
                  <span className="text-accent-200">{getCurrencyIcon(rate.code)}</span>
                  <span className="text-sm font-medium">{rate.code}</span>
                  <span className="text-sm text-accent-100">
                    ₱{rate.buyingRate.toFixed(2)} / ₱{rate.sellingRate.toFixed(2)}
                  </span>
                </div>
              ))}
            </div>
          ...
          <div className="flex items-center space-x-6 pl-6 border-l border-accent-500">
            {weatherData.map((data) => (
              <div key={data.location} className="flex items-center space-x-2">
                <span className="text-xs font-medium text-accent-100">{data.location}</span>
