...
export default function HouseMembersPage() {
  ...
  const houseData = legislativeData.find(
    (item: any) => item.chamber.includes('House of Representatives')
  )
  ...
  const filteredMembers = useMemo(() => {
    return houseMembers.filter((member: HouseMember) => {
      ...
      const matchesProvince = !selectedProvince || member.province_city === selectedProvince
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            {houseMembers.length} Representatives from {provinces.length} provinces/cities
          </p>
        </div>
      ...
      {Object.keys(membersByProvince).length === 0 ? (
        ...
        <div className="space-y-6">
          {Object.entries(membersByProvince).map(([province, members]) => (
            <div key={province} className="bg-white rounded-lg border overflow-hidden">
              <div className="bg-gray-50 px-4 py-3 border-b">
                <h2 className="text-lg font-medium text-gray-900">{province}</h2>
                <p className="text-sm text-gray-500">{members.length} {members.length === 1 ? 'representative' : 'representatives'}</p>
              </div>
              
              <div className="divide-y">
                {members.map((member, index) => (
                  <div key={index} className="p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-2">
                    <div>
