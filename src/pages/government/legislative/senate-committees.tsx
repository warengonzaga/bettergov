...
export default function SenateCommitteesPage() {
  ...
  // Get Senate data
  const senateData = legislativeData.find(
    (item: any) => item.chamber.includes('Senate')
  )
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredCommittees.map((committee: Committee, index) => (
            <div key={index} className="bg-white rounded-lg border overflow-hidden">
              <div className="p-4">
                <h3 className="font-medium text-gray-900 mb-2">{committee.committee}</h3>
                <div className="flex items-start">
                  <div className="bg-primary-50 text-primary-700 text-xs font-medium px-2 py-1 rounded-md">
                    Chairperson
                  </div>
                  <p className="ml-2 text-sm text-gray-700">{committee.chairperson}</p>
                </div>
              </div>
            </div>
          ))}
