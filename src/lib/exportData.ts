 ...
 */
export async function exportMeilisearchData(options: ExportMeilisearchOptions): Promise<void> {
  try {
    const { host, port, apiKey, indexName, filters, searchTerm = '', filename = 'flood_control_projects' } = options;
    
    // Fetch data from Meilisearch
    const response = await fetch(`${host}:${port}/indexes/${indexName}/search`, {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        q: searchTerm,
        filter: filters,
        limit: 1000 // Get a large number of results for export
      })
    });
